...
Two more hooks push memory without a `recall` call, each printing a `💡 qmemd` block around a Bash command — read either with `qmemd show <slug>`. The **beacon** (`PreToolUse`/Bash) fires a once-per-repo-per-session pivot block on the first Bash call in a repo (repo + global fact counts, then up to ten facts or a tag histogram), plus an overlap block whenever a command's tokens match a fact's tags/slug; each fact surfaces at most once per session. The **probe** (`PostToolUseFailure`/Bash) fires only where the host wires that event — the Claude Code plugin today; Codex CLI and Cursor ship no probe entry — and after a failed Bash call it names facts matching the command and its first error line; the same failure probes once per session. `qmemd hook stats` reports how often each kind fires and gets acted on.
