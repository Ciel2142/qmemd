...
Fact replacements flush a temporary file before publishing it atomically, so readers see a complete old or new file. Fact-editing commands coordinate across local processes using a per-root lock; a concurrent slug collision is checked again after the earlier writer finishes. Writers wait up to 30 seconds, while synchronous `doctor --fix` reports contention immediately. The internal `.qmemd-write-lock/` directory is excluded from Git; locks release automatically when a process exits. Cross-machine changes still follow the Git sync rules.
...
When project, type, or platform filtering leaves a hybrid result short, recall retrieves additional lexical candidates and reranks the missing in-scope facts using the same relevance floor. qmd's internal source limits can still omit semantic candidates, so `saturated:true` also means completeness is unknown, even on a short page. A supplemental rerank failure preserves the existing hybrid hits and reports `degraded:true`.
