...
- **Write scope defaults per type** (qmemd-due): CLI `remember` and stdio MCP `remember` without an explicit `project` scope `project`/`reference` facts to `basename(cwd)` and `user`/`feedback` to `global` (`resolveWriteScope`/`defaultProjectFor` in `scope.ts`). On `--replace`, an unresolved `project` passes through unchanged so the engine keeps the fact's stored scope instead of re-homing it; an explicit `project` on `--replace` or `--force` still wins over the stored scope. Daemon MCP (`--http`) `remember` and REST `POST /remember` require `project` explicitly, since there is no cwd to default from server-side: a missing/blank value is a schema-level `isError` (MCP) or `400 {"error":"Missing required field: project"}` (REST), not a silent global write. `engine.ts` throws `project is required for a new fact; pass a repo name or "global"` if a new fact somehow reaches it without one.
- **`rescope` migrates scope, not model-driven:** `qmemd rescope` plans by default (dry run, filesystem-only) and applies all-or-nothing in one commit; revert an apply via `git revert <commit>` in `$QMD_MEMORY_DIR` + `qmemd reindex`. `user`/`feedback` facts are never touched.
