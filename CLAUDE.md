...
- **Write scope defaults per type** (qmemd-due): CLI `remember` and stdio MCP `remember` without an explicit `project` scope `project`/`reference` facts to `basename(cwd)` and `user`/`feedback` to `global` (`resolveWriteScope`/`defaultProjectFor` in `scope.ts`). `--replace` passes an unresolved `project` through so the engine keeps the fact's stored scope rather than re-homing it. Daemon MCP (`--http`) `remember` and REST `POST /remember` require `project` explicitly — no cwd to default from server-side; a missing/blank value is a schema-level `isError` (MCP) or `400 {"error":"Missing required field: project"}` (REST), not a silent global write. `engine.ts` throws `project is required for a new fact; pass a repo name or "global"` if a new fact somehow reaches it without one.
